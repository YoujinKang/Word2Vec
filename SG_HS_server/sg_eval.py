...
def count_in_top4(similarity, word_to_id, valid):
    count = 0
    top4_idx = []
    top4_sim = []
    for i in range(len(similarity)):
        ...
        for j in range(4):  # temp에 담긴 4개의 단어를 하나씩 훑는다. 
            ...
            if pred in val_set:  # 그 단어가 문장 내에 있어야하고
...
parser.add_argument('--log_dir', type=str, default='./SG_HS/train_log/SG_HS_sub1e-05_window10_dim300_5epoch/weight')
parser.add_argument('--weight', type=str, default='ckpt_weight26.pkl')
...
for i in range(args.data_seg):
    ...
    syn_max_top4, syn_sim_top4, syn_count = count_in_top4(similarity_syn, word_to_id, batch_syn)
    ...
    sem_max_top4, sem_sim_top4, sem_count = count_in_top4(similarity_sem, word_to_id, batch_sem)
...
print("time passed: ", (time1 - time0)/60, " min")
