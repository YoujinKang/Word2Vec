...
def test():
    word_to_id = {'<\s>': 0, 'b': 1, 'c ': 2, 'd': 3, 'e': 4, 'a': 5, 'f':6, 'g':7}
    vocab = {'<\s>': 0, "a": 1, 'b': 6, 'c': 3, 'd': 2, 'e': 2, 'f': 1, 'g': 1}
    huffman = HuffmanTree()
    ...
    sentence = [5, 1, 1, 1, 7, 1, 1, 1, 2, 2, 2, 3, 3, 4, 6, 4]
    V = len(vocab)
    embedding_size = 10
    window_size = 5
    target_idx = 1
    ...
    lr = 0.1
    model = ModelSG_HS(V, embedding_size, code_index, code_sign, code_index_len)
