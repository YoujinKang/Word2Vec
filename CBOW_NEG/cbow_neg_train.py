# from cbow_neg_model import ModelCBOW_Neg
from re_model import ModelCBOW_Neg
import argparse
import numpy as np
...
from common.functions import subsampling_prob, subsampling, make_contexts, unigramsampler


parser = argparse.ArgumentParser()

parser.add_argument('--model', type=str, default='CBOW_Neg')
...
log_dir = 'CBOW_NEG/train_log/{}{}_sub{}_window{}_dim{}_{}epoch/'.format(args.model, args.negative_num, args.sub_t, args.window_size, args.embedding_size, args.max_epoch)
...
sample_table = []
sample_table = unigramsampler(vocabulary, word_to_id, power=3/4)
sample_table = np.array(sample_table)


model = ModelCBOW_Neg(V, args.embedding_size, sample_table, args.negative_num)
...
for epoch in range(1, args.max_epoch):
        ...
        for i in tqdm(range(len(sentence_list)), desc='file {}/100'.format(j+1), bar_format="{l_bar}{bar:10}{r_bar}"):
            ...
            for center_idx, center in enumerate(sentence):
                ...
                if (args.eval_interval is not None) and (num_trained_word % args.eval_interval ==1):
                    ...
                    tb_writer.add_scalar('score/real_train_word(*{})'.format(args.eval_interval), avg_score, num_trained_word)
                    tb_writer.add_scalar('loss/real_train_word(*{})'.format(args.eval_interval), avg_loss, num_trained_word)
    ...
    with open(os.path.join(weight_dir, '{}{}_sub{}_window{}_dim{}_{}epoch.pkl'
            .format(args.model, args.negative_num, args.sub_t, args.window_size, args.embedding_size, epoch+1)), 'wb') as fw:
        pickle.dump((model.W_in, model.W_out), fw)
        fw.close()